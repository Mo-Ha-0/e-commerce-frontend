...
export function useLowStock(threshold = 5) {
  return useQuery({
    ...
    queryFn: () =>
      api.get(`${ENDPOINTS.INVENTORY}/low-stock`, { params: { threshold } })
        .then((res) => res.data),
  })
}
...
export function useInventoryLogs() {
  return useQuery({
    ...
    queryFn: () =>
      api.get<InventoryLog[]>(`${ENDPOINTS.INVENTORY}/logs`).then((res) => res.data),
  })
}
...
export function useUpdateStock() {
  ...
  return useMutation({
    mutationFn: ({ productId, dto }: { productId: string; dto: UpdateStockDto }) =>
...
export function useRestock() {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ productId, dto }: { productId: string; dto: RestockDto }) =>
